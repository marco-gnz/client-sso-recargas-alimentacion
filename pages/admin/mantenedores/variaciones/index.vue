<template>
  <div>
    <HeroPage namepage="Tipos de ausentismo / Tipos de incremento" />
                         ...
                         <table class="table is-narrow is-hoverable is-fullwidth" v-loading="loadingData">
                            ...
                            <tbody>
                              <tr v-for="(element, index) in data" :key="index">
                                ...
                                <td>{{element.sigla ? element.sigla : '--'}}</td>
                                <td>{{element.name ? element.name : '--'}}</td>
                                <td>
                                  <nuxt-link v-if="hasPermission('mantenedor.update')" @click.native="editContractual(index)" :to="{path: `/admin/mantenedores/variaciones`, query: {tipo:currentRouteQuery, id: element.id }}"><el-button size="mini" type="primary" icon="el-icon-edit" circle></el-button></nuxt-link>
                                </td>
                              </tr>
                            </tbody>
                          </table>
...
import MenuVariaciones from '../../../../components/admin/mantenedores/Menu-variaciones.vue';
import ModalAddVariacion from '../../../../components/admin/mantenedores/ModalAddVariacion.vue';
import ModalEditVariacion from '../../../../components/admin/mantenedores/ModalEditVariacion.vue';
import HeroPage from '../../../../components/admin/usuarios/HeroPage.vue'
export default {
  components: { HeroPage, MenuVariaciones, ModalAddVariacion, ModalEditVariacion },
  middleware: "auth",
  head() {
    return {
        title: `Variaciones`
  ...
  methods:{
    ...
    keySearchInput:function(){
      if(this.filtroInput.length > 1){
          ...
          this.setTimeoutBuscador = setTimeout(this.actionGetVariaciones, 500);
      }
    },
