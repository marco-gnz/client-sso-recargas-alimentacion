          ...
          <div class="column">
            <div class="field">
              <div class="control is-medium" :class="(loadingTablaResumen && input_query.length ? 'is-loading' : '')">
                <el-input autofocus placeholder="Buscar funcionarios por rut, nombre o apellidos" v-model="input_query"
            ...
            <div class="field">
              <div class="columns">
                <div class="column">
                  ...
                  <el-select size="mini" v-model="filtro_leyes" multiple collapse-tags clearable
                    placeholder="Seleccione ley" class="element-select">
                    <el-option v-for="ley in leyes" :key="ley.id" :label="ley.nombre" :value="ley.id">
                    </el-option>
                  </el-select>
                </div>
                <div class="column">
                  <label class="label">C. costo</label>
                  <el-select size="mini" v-model="filtro_centro_costo" multiple collapse-tags clearable
                    placeholder="Seleccione centro" class="element-select">
                    <el-option v-for="centro in centroCostosRecarga" :key="centro" :label="centro" :value="centro">
                    </el-option>
                  </el-select>
                </div>
                <div class="column">
                  <div class="field">
                    <div class="columns">
                      <div class="column">
                        ...
                        <el-select size="mini" v-model="filtro_unidad" filterable multiple collapse-tags clearable
                          placeholder="Seleccione unidad" class="element-select">
                          <el-option v-for="unidad in unidades" :key="unidad.id" :label="unidad.nombre"
                            :value="unidad.id">
                          </el-option>
                        </el-select>
                      </div>
                      <div class="column pt-6">
                        <el-checkbox :disabled="!filtro_unidad.length" v-model="equals_unidad">{{ equals_unidad ? '=' :
                          '!=' }}</el-checkbox>
                      </div>
                    </div>
                  </div>
                ...
                <div class="column">
                  ...
                  <el-select size="mini" v-model="filtro_horas" filterable multiple collapse-tags clearable
                    placeholder="Seleccione hora" class="element-select">
                    <el-option v-for="hora in horas" :key="hora.id" :label="hora.nombre" :value="hora.id">
                    </el-option>
                  </el-select>
                </div>
                <div class="column">
                  <label class="label">Tipo de ausentismo</label>
                  <el-select size="mini" v-model="filtro_tipo_ausentismo" filterable multiple collapse-tags clearable
                    placeholder="Seleccione tipo" class="element-select">
                    <el-option v-for="tipo in tipoAusentismos" :key="tipo.id" :label="tipo.nombre" :value="tipo.id">
                    </el-option>
                  </el-select>
                </div>
                <div class="column">
                  <button class="button is-link mt-5" :class="(loadingTablaResumen ? 'is-loading' : '')"
                    @click.prevent="actionGetFuncionariosFiltro">
            ...
            <div class="table-container pt-2" v-loading.fullscreen.lock="loadingTablaResumen && pagination.total <= 0"
              ...
              <template v-if="(pagination.total > 0)">
                <table class="table is-bordered  is-narrow is-hoverable is-fullwidth" v-loading="loadingTablaResumen"
                  element-loading-text="Cargando resultados...">
                  <thead>
                    <tr>
                      <th>
                        ...
                        <div class="columns">
                          <div class="column">
                            <el-popover placement="top" width="300" v-model="visible" @hide="hideTop"
                              @after-enter="hideTop" @after-leave="hideTop">
                              <p>Marcados({{ esquemas_checked.length }}):</p>
                              <div style="text-align: center; margin: 0">
                                <el-button :disabled="loadingMarcados" size="mini" type="danger"
                                  @click="statusEsquemas(false)">Deshabilitar</el-button>
                                <el-button :disabled="loadingMarcados" type="primary" size="mini"
                                  @click="statusEsquemas(true)">Habilitar</el-button>
                              </div>
                              <el-checkbox slot="reference" :indeterminate="isIndeterminate" v-model="checked_all"
                      ...
                      <th>
                        <div class="columns">
                          <div class="column">
                            <span class="click" @click.prevent="orderByData(ordenamientos[3])">Tur</span>
                          </div>
                          <div class="column">
                      ...
                      <th>
                        <div class="columns">
                          <div class="column">
                            <span class="click" @click.prevent="orderByData(ordenamientos[4])">Contrato</span>
                          </div>
                          <div class="column">
                      ...
                      <th>
                        <div class="columns">
                          ...
                          <div class="column">
                            ...
                            <i v-if="isParamDESC(ordenamientos[5])" class="el-icon-caret-bottom"></i>
                          </div>
                        </div>
                      </th>
                      <th>
                        <div class="columns">
                          ...
                          <div class="column">
                            ...
                            <i v-if="isParamDESC(ordenamientos[6])" class="el-icon-caret-bottom"></i>
                          </div>
                        </div>
                      </th>
                      <th>
                        <div class="columns">
                          <div class="column">
                            <span class="click" @click.prevent="orderByData(ordenamientos[7])">T L+N</span>
                          </div>
                          <div class="column">
                            <i v-if="isParamASC(ordenamientos[7])" class="el-icon-caret-top"></i>
                            <i v-if="isParamDESC(ordenamientos[7])" class="el-icon-caret-bottom"></i>
                          </div>
                        </div>
                      </th>
                      <template v-if="recarga.total_grupos">
                        <th :class="(open_grupo === 1 ? 'has-background-grey-lighter' : '')">
                          <el-link :underline="false" @click.prevent="openGrupo(1)"
                            :type="open_grupo === 1 ? 'warning' : 'info'">1 <i
                              :class="open_grupo === 1 ? 'el-icon-minus' : 'el-icon-plus'"></i></el-link>
                        </th>
                        <template v-if="open_grupo != undefined && open_grupo === 1">
                          <td v-for="(tipo, index) in recarga.total_grupos" :key="index"
                        ...
                        <th :class="(open_grupo === 2 ? 'has-background-grey-lighter' : '')">
                          <el-link :underline="false" @click.prevent="openGrupo(2)"
                            :type="open_grupo === 2 ? 'warning' : 'info'">2
                            <i :class="open_grupo === 2 ? 'el-icon-minus' : 'el-icon-plus'"></i></el-link>
                        </th>
                        ...
                        <th :class="(open_grupo === 3 ? 'has-background-grey-lighter' : '')">
                          <el-link :underline="false" @click.prevent="openGrupo(3)"
                            :type="open_grupo === 3 ? 'warning' : 'info'">3
                            <i :class="open_grupo === 3 ? 'el-icon-minus' : 'el-icon-plus'"></i></el-link>
                        </th>
                      ...
                      <th>
                        <div class="columns">
                          <div class="column">
                            <span class="click" @click.prevent="orderByData(ordenamientos[11])">Viá</span>
                          </div>
                          <div class="column">
                      ...
                      <th>
                        <div class="columns">
                          <div class="column">
                            <span class="click" @click.prevent="orderByData(ordenamientos[12])">A. días</span>
                          </div>
                          <div class="column">
                      ...
                      <th>
                        <div class="columns">
                          <div class="column">
                            <span class="click" @click.prevent="orderByData(ordenamientos[13])">DC</span>
                          </div>
                          <div class="column">
                            <i v-if="isParamASC(ordenamientos[13])" class="el-icon-caret-top"></i>
                            <i v-if="isParamDESC(ordenamientos[13])" class="el-icon-caret-bottom"></i>
                          </div>
                        </div>
                      </th>
                      <th>
                        <div class="columns">
                          <div class="column">
                            <span class="click" @click.prevent="orderByData(ordenamientos[14])">A. $</span>
                          </div>
                          <div class="column">
                            <i v-if="isParamASC(ordenamientos[14])" class="el-icon-caret-top"></i>
                            <i v-if="isParamDESC(ordenamientos[14])" class="el-icon-caret-bottom"></i>
                          </div>
                        </div>
                      </th>
                      <th>
                        <div class="columns">
                          <div class="column">
                            <span class="click" @click.prevent="orderByData(ordenamientos[15])">MC</span>
                          </div>
                          <div class="column">
                            <i v-if="isParamASC(ordenamientos[15])" class="el-icon-caret-top"></i>
                            <i v-if="isParamDESC(ordenamientos[15])" class="el-icon-caret-bottom"></i>
                          </div>
                        </div>
                      </th>
                    ...
                    <tr>
                      <th>
                        <template v-if="recarga.last_status_value === 0 && hasPermission('esquema.delete')">
                          <el-popconfirm @confirm="deleteData('esquema')" confirm-button-type="warning"
                            confirm-button-text='Si, eliminar' cancel-button-text='No' icon="el-icon-info"
                            icon-color="warning" title="¿Eliminar cartolas?">
                            <el-button :class="(loadingDeleteData ? 'is-loading' : '')" size="mini" type="danger"
                        ...
                        <div class="columns">
                          <div class="column">
                            <template v-if="recarga.last_status_value === 0 && hasPermission('asignacion.delete')">
                              <el-popconfirm @confirm="deleteData('asignacion')" confirm-button-type="warning"
                                confirm-button-text='Si, eliminar' cancel-button-text='No' icon="el-icon-info"
                                icon-color="warning" title="¿Eliminar asignaciones?">
                                <el-button :class="(loadingDeleteData ? 'is-loading' : '')" size="mini" type="danger"
                                  icon="el-icon-delete" circle slot="reference"></el-button>
                              </el-popconfirm>
                            </template>
                        ...
                        <div class="columns">
                          ...
                          <div class="column">
                            <template v-if="recarga.last_status_value === 0 && hasPermission('contrato.delete')">
                              <el-popconfirm @confirm="deleteData('contrato')" confirm-button-type="warning"
                                confirm-button-text='Si, eliminar' cancel-button-text='No' icon="el-icon-info"
                                icon-color="warning"
                        ...
                        <div class="columns">
                          ...
                          <div class="column">
                            <template v-if="recarga.last_status_value === 0 && hasPermission('turno.delete')">
                              <el-popconfirm @confirm="deleteData('asistencias')" confirm-button-type="warning"
                                confirm-button-text='Si, eliminar' cancel-button-text='No' icon="el-icon-info"
                                icon-color="warning" title="¿Eliminar turnos?">
                                <el-button :class="(loadingDeleteData ? 'is-loading' : '')" size="mini" type="danger"
                                  icon="el-icon-delete" circle slot="reference"></el-button>
                              </el-popconfirm>
                            </template>
                        ...
                        <div class="columns">
                          ...
                          <div class="column">
                            <template v-if="recarga.last_status_value === 0 && hasPermission('ausentismo.delete')">
                              <el-popconfirm @confirm="deleteData('ausentismos_grupo_uno')"
                                confirm-button-type="warning" confirm-button-text='Si, eliminar' cancel-button-text='No'
                                icon="el-icon-info" icon-color="warning" title="¿Eliminar ausentismos de grupo 1?">
                                <el-button :class="(loadingDeleteData ? 'is-loading' : '')" size="mini" type="danger"
                                  icon="el-icon-delete" circle slot="reference"></el-button>
                              </el-popconfirm>
                            </template>
                      ...
                      <th>
                        <div class="columns">
                          ...
                          <div class="column">
                            <template v-if="recarga.last_status_value === 0 && hasPermission('ausentismo.delete')">
                              <el-popconfirm @confirm="deleteData('ausentismos_grupo_dos')"
                                confirm-button-type="warning" confirm-button-text='Si, eliminar' cancel-button-text='No'
                                icon="el-icon-info" icon-color="warning" title="¿Eliminar ausentismos de grupo 2?">
                                <el-button :class="(loadingDeleteData ? 'is-loading' : '')" size="mini" type="danger"
                                  icon="el-icon-delete" circle slot="reference"></el-button>
                              </el-popconfirm>
                            </template>
                      ...
                      <th>
                        <div class="columns">
                          ...
                          <div class="column">
                            <template v-if="recarga.last_status_value === 0 && hasPermission('ausentismo.delete')">
                              <el-popconfirm @confirm="deleteData('ausentismos_grupo_tres')"
                                confirm-button-type="warning" confirm-button-text='Si, eliminar' cancel-button-text='No'
                                icon="el-icon-info" icon-color="warning" title="¿Eliminar ausentismos de grupo 3?">
                                <el-button :class="(loadingDeleteData ? 'is-loading' : '')" size="mini" type="danger"
                                  icon="el-icon-delete" circle slot="reference"></el-button>
                              </el-popconfirm>
                            </template>
                        ...
                        <div class="columns">
                          ...
                          <div class="column">
                            <template v-if="recarga.last_status_value === 0 && hasPermission('viatico.delete')">
                              <el-popconfirm @confirm="deleteData('viatico')" confirm-button-type="warning"
                                confirm-button-text='Si, eliminar' cancel-button-text='No' icon="el-icon-info"
                                icon-color="warning" title="¿Eliminar viáticos?">
                                <el-button :class="(loadingDeleteData ? 'is-loading' : '')" size="mini" type="danger"
                                  icon="el-icon-delete" circle slot="reference"></el-button>
                              </el-popconfirm>
                            </template>
                      ...
                      <th>
                        <div class="columns">
                          ...
                          <div class="column">
                            <template v-if="recarga.last_status_value === 0 && hasPermission('ajuste.create.masivo')">
                              <el-popconfirm @confirm="deleteData('reajuste')" confirm-button-type="warning"
                                confirm-button-text='Si, eliminar' cancel-button-text='No' icon="el-icon-info"
                                icon-color="warning" title="¿Eliminar reajustes masivos?">
                                <el-button :class="(loadingDeleteData ? 'is-loading' : '')" size="mini" type="danger"
                                  icon="el-icon-delete" circle slot="reference"></el-button>
                              </el-popconfirm>
                            </template>
                  ...
                  <tbody>
                    <tr v-for="(funcionario, index) in funcionarios" :key="index" @click.prevent="clickItem(index)"
                      :class="(index_selected_table === index ? 'row-selected' : '')">
                      <td>
                          ...
                          <div class="columns">
                            <div class="column">
                              <el-tooltip :content="`${!funcionario.beneficio ? `No` : `Si`}`" placement="top">
                                <el-switch v-loading.lock="loadingIndex && index_click_funcionario === index"
                                  :disabled="(loadingIndex) || (funcionario.checked)"
                                  @change="editBeneficio(index, funcionario.id)" :active-value="!funcionario.beneficio"
                                  :inactive-value="funcionario.beneficio" inactive-color="#F1948A"
                                  active-color="#13ce66"></el-switch>
                              </el-tooltip>
                            </div>
                            <div class="column">
                              <el-checkbox v-model="funcionario.checked"
                                @click.native="toggleFuncionarioCheck(funcionario)"></el-checkbox>
                            </div>
                          </div>
                        </template>
                        <template v-else>
                          <el-tag effect="dark" size="mini" :type="(!funcionario.beneficio ? 'danger' : 'success')"
                            disable-transitions>{{ !funcionario.beneficio ? 'No' : 'Si' }}</el-tag>
                        </template>
                      </td>
                      <td>
                        <template v-if="recarga.last_status_value === 0 && hasPermission('esquema.update')">
                          <el-checkbox v-loading.lock="loadingIndex && index_click_funcionario_reemplazo === index"
                            :value="funcionario.es_remplazo" @click.native="editReemplazo(index, funcionario.id)">{{
                              funcionario.es_remplazo ? 'Si' :
                                'No' }}</el-checkbox>
                        </template>
                        <template v-else>
                          <el-tag effect="dark" size="mini" :type="(funcionario.es_remplazo ? 'warning' : 'primary')"
                            disable-transitions>{{ funcionario.es_remplazo ? 'Si' : 'No' }}</el-tag>
                        </template>
                      </td>
                      <td>
                        <template v-if="!open_grupo">
                          <p :class="(funcionario.tipo_ingreso ? 'has-background-warning-light' : '')">
                            {{ funcionario.funcionario_apellidos ? funcionario.funcionario_apellidos : '--' }}
                            {{ funcionario.funcionario_nombre ? funcionario.funcionario_nombre : '--' }}</p>
                        </template>
                        <template v-else>
                          <p :class="(funcionario.tipo_ingreso ? 'has-background-warning-light' : '')">
                            {{ funcionario.funcionario_apellidos ? funcionario.funcionario_apellidos : '--' }}</p>
                        </template>
                      </td>
                      <td>
                        <nuxt-link :to="`/admin/esquemas/${funcionario.uuid}/asignaciones`">
                          <el-tag effect="dark" size="mini" :type="(funcionario.turno_asignacion ? 'warning' : 'info')"
                            disable-transitions>{{ funcionario.turno_asignacion ? 'Si' : 'No' }}</el-tag>
                        </nuxt-link>
                      </td>
                      <td>
                        <template v-if="recarga.last_status_value === 0 && hasPermission('esquema.update')">
                          <el-tooltip :content="`${funcionario.es_turnante_nombre}`" placement="top">
                            <el-switch v-loading.lock="loadingIndex && index_click_funcionario === index"
                              :disabled="(loadingIndex)" @change="editTurno(index, funcionario.id)"
                              :active-value="!funcionario.es_turnante_value"
                              :inactive-value="funcionario.es_turnante_value" inactive-color="#e9ae55"
                              active-color="#0e6db8"></el-switch>
                          </el-tooltip>
                        </template>
                        <template v-else>
                          <el-tag effect="dark" size="mini" :type="(funcionario.es_turnante_type)"
                            disable-transitions>{{ funcionario.es_turnante_nombre }}</el-tag>
                        </template>
                      </td>
                      <td>
                        <div class="columns">
                          <div class="column">
                            <el-tooltip :disabled="(funcionario.contrato_n_registros <= 0 ? true : false)" class="item"
                              effect="dark"
                              :content="`${funcionario.contrato_n_registros} ${funcionario.contrato_n_registros > 1 ? 'registros' : 'registro'}`"
                              placement="top-start">
                              <nuxt-link :to="`/admin/esquemas/${funcionario.uuid}/contratos`">
                                <p
                                  :class="(funcionario.total_dias_contrato === 0 || funcionario.total_dias_contrato < recarga.total_dias_habiles_beneficio ? 'has-text-danger has-text-weight-semibold' : '')">
                                  {{ funcionario.total_dias_contrato }}</p>
                              </nuxt-link>
                            </el-tooltip>
                          </div>
                          <div class="column">
                      ...
                      <td>
                        <div class="columns">
                          <div class="column">
                            <el-tooltip :disabled="!funcionario.unidad_nom"
                              :content="`${funcionario.unidad_nom ? funcionario.unidad_nom : '--'}`"
                              placement="top-start">
                      ...
                      <td>
                        <div class="columns">
                          <div class="column">
                            <p>{{ funcionario.c_costo ? funcionario.c_costo : '--' }}</p>
                          </div>
                        </div>
                      </td>
                      <td>
                        <nuxt-link :to="`/admin/esquemas/${funcionario.uuid}/turnos`">
                          <el-popover
                            ...
                            placement="top-end" width="150" trigger="hover">
                            <el-descriptions title="Turnos">
                              <el-descriptions-item label="L">{{ funcionario.total_dias_turno_largo_en_periodo_contrato
                                }}</el-descriptions-item>
                              <el-descriptions-item label="N">{{
                                funcionario.total_dias_turno_nocturno_en_periodo_contrato }}</el-descriptions-item>
                              <el-descriptions-item label="X">{{ funcionario.total_dias_libres_en_periodo_contrato
                                }}</el-descriptions-item>
                            </el-descriptions>
                            <span slot="reference"
                              :class="(funcionario.es_turnante && funcionario.total_turno > 0 ? 'has-text-success' : (funcionario.es_turnante && funcionario.total_turno <= 0 ? 'has-text-danger' : 'has-text-black'))">{{
                                funcionario.total_turno }}</span>
                          </el-popover>
                        </nuxt-link>
                      </td>
                      <td :class="(open_grupo === 1 ? 'has-background-grey-lighter' : '')">
                        <el-tooltip :disabled="(funcionario.total_dias_grupo_uno <= 0 ? true : false)" class="item"
                          effect="dark"
                          :content="`${funcionario.grupo_uno_n_registros} ${funcionario.grupo_uno_n_registros > 1 ? 'registros' : 'registro'}`"
                          placement="top-start">
                          <nuxt-link :to="`/admin/esquemas/${funcionario.uuid}/ausentismos?grupo=${1}`">
                            <strong
                              :class="funcionario.total_dias_grupo_uno > recarga.total_dias_mes_calculo || funcionario.total_dias_grupo_uno <= 0 ? 'has-text-danger' : 'has-text-success'">{{
                                funcionario.total_dias_grupo_uno }}</strong>
                          </nuxt-link>
                        </el-tooltip>
                      </td>
                      <template v-if="(open_grupo === 1) && (funcionario.ausentismos_grupo)">
                        <td class="is-bordered has-background-white-ter"
                          v-for="(grupo, index) in funcionario.ausentismos_grupo" :key="index"><span
                            :class="(grupo.total_dias <= 0 ? 'has-text-danger' : 'has-text-success-dark has-text-weight-semibold')">{{
                              grupo.total_dias }}</span>
                        </td>
                      </template>
                      <td :class="(open_grupo === 2 ? 'has-background-grey-lighter' : '')">
                        <el-tooltip :disabled="(funcionario.total_dias_grupo_dos <= 0 ? true : false)" class="item"
                          effect="dark"
                          :content="`${funcionario.grupo_dos_n_registros} ${funcionario.grupo_dos_n_registros > 1 ? 'registros' : 'registro'}`"
                          placement="top-start">
                          <nuxt-link :to="`/admin/esquemas/${funcionario.uuid}/ausentismos?grupo=${2}`">
                            <strong
                              :class="funcionario.total_dias_grupo_dos > recarga.total_dias_mes_calculo || funcionario.total_dias_grupo_dos <= 0 ? 'has-text-danger' : 'has-text-success'">{{
                                funcionario.total_dias_grupo_dos }}</strong>
                          </nuxt-link>
                        </el-tooltip>
                      </td>
                      ...
                      <td :class="(open_grupo === 3 ? 'has-background-grey-lighter' : '')">
                        <el-tooltip :disabled="(funcionario.total_dias_grupo_tres <= 0 ? true : false)" class="item"
                          effect="dark"
                          :content="`${funcionario.grupo_tres_n_registros} ${funcionario.grupo_tres_n_registros > 1 ? 'registros' : 'registro'}`"
                          placement="top-start">
                          <nuxt-link :to="`/admin/esquemas/${funcionario.uuid}/ausentismos?grupo=${3}`">
                            <strong
                              :class="funcionario.total_dias_grupo_tres > recarga.total_dias_mes_calculo || funcionario.total_dias_grupo_tres <= 0 ? 'has-text-danger' : 'has-text-success'">{{
                                funcionario.total_dias_grupo_tres }}</strong>
                          </nuxt-link>
                        </el-tooltip>
                      </td>
                      ...
                      <td>
                        <nuxt-link :to="`/admin/esquemas/${funcionario.uuid}/viaticos`">
                          <span
                            :class="(funcionario.calculo_viaticos > 0 ? 'has-text-danger-dark has-text-weight-semibold' : '')">{{
                              funcionario.calculo_viaticos }}</span>
                        </nuxt-link>
                      </td>
                      <td style="border-bottom: 1px solid; border-color: rgb(14,109,184, 0.5);">
                        <div class="columns">
                          <div class="column is-one-quarter">
                            <el-badge :value="funcionario.ajustes_dias_n_registros" :max="10" class="item"
                              type="warning" :hidden="(funcionario.ajustes_dias_n_registros > 0 ? false : true)">
                              <nuxt-link :to="`/admin/esquemas/${funcionario.uuid}/ajustes`"><span
                                  :class="(funcionario.calculo_dias_ajustes <= 0 ? 'has-text-dark' : 'has-text-danger has-text-weight-semibold')">{{
                                    funcionario.calculo_dias_ajustes }}</span></nuxt-link>
                            </el-badge>
                          </div>
                          <template v-if="recarga.last_status_value === 0 && hasPermission('ajuste.create')">
                            <div class="column">
                              <button class="button is-small"
                                @click.prevent="openModalAddReajuste(index, funcionario, 0)">
                                <span class="icon is-small">
                      ...
                      <td><span
                          :class="(funcionario.total_dias_cancelar <= 0 || funcionario.total_dias_cancelar < recarga.total_dias_habiles_beneficio ? 'has-text-danger' : (funcionario.total_dias_cancelar > recarga.total_dias_habiles_beneficio ? 'has-background-danger-dark has-text-weight-semibold has-text-white' : ''))">{{
                            funcionario.total_dias_cancelar }}</span>
                      </td>
                      <td style="border-bottom: 1px solid; border-color: rgb(14,109,184, 0.5);">
                        <div class="columns">
                          <div class="column">
                            <el-badge :value="funcionario.ajustes_monto_n_registros" :max="10" class="item"
                              type="warning" :hidden="(funcionario.ajustes_monto_n_registros > 0 ? false : true)">
                              <nuxt-link :to="`/admin/esquemas/${funcionario.uuid}/ajustes`"><span
                                  :class="(funcionario.total_monto_ajuste <= 0 ? 'has-text-dark' : 'has-text-danger has-text-weight-semibold')">{{
                                    funcionario.total_monto_ajuste }}</span></nuxt-link>
                            </el-badge>
                          </div>
                          <template v-if="recarga.last_status_value === 0 && hasPermission('ajuste.create')">
                            <div class="column">
                              <button class="button is-small"
                                @click.prevent="openModalAddReajuste(index, funcionario, 1)">
                                <span class="icon is-small">
                                  <i class="el-icon-plus"></i>
                                </span>
                              </button>
                      ...
                      <td><strong
                          :class="(funcionario.monto_cancelar.monto_total_cancelar_value <= 0 ? 'has-text-danger' : (funcionario.monto_cancelar.monto_total_cancelar_value > recarga.monto_estimado_no_turnante ? 'has-background-danger-dark has-text-weight-semibold has-text-white' : 'has-text-success'))">{{
                            funcionario.monto_cancelar.monto_total_cancelar_format }}</strong></td>
                      <td>
                        <nuxt-link v-if="hasPermission('esquema.read')"
                          :to="`/admin/esquemas/${funcionario.uuid}/detalle`"><el-button size="mini" type="primary"
                            icon="el-icon-view" circle></el-button></nuxt-link>
                        <template
                          v-if="funcionario.tipo_ingreso && recarga.last_status_value === 0 && hasPermission('esquema.delete')">
                          <el-popconfirm @confirm="deleteEsquema(index, funcionario.id)" confirm-button-type="warning"
                            confirm-button-text='Si, eliminar' cancel-button-text='No' icon="el-icon-info"
                            icon-color="warning" title="¿Eliminar cartola? Los registros asociados no se eliminarán.">
                            <el-button :class="(loadingDeleteData ? 'is-loading' : '')" size="mini" type="danger"
                              icon="el-icon-delete" circle slot="reference"></el-button>
                          </el-popconfirm>
                        </template>
                        <el-button v-if="funcionario.errores.length" type="danger" size="mini"
                          icon="el-icon-message-solid" class="click" circle
                          @click.prevent="showErrores(index, funcionario.errores, funcionario.advertencias)"></el-button>
                        <el-button v-if="funcionario.advertencias.length" type="warning" size="mini"
                          icon="el-icon-message-solid" class="click" circle
                          @click.prevent="showErrores(index, funcionario.errores, funcionario.advertencias)"></el-button>
                      </td>
                    </tr>
            ...
            <div class="buttons">
              ...
              <button v-if="(recarga.last_status_value === 0 || recarga.last_status_value === 1)"
                class="button is-is-success is-outlined" @click.prevent="openModalPublicarPlanilla">
                <span>Publicar recarga</span>
                <span class="icon is-small">
                  <i class="el-icon-success"></i>
                </span>
              </button>
              <button :class="(loadingSendCartolas ? 'is-loading' : '')" @click.prevent="sendCartolas"
                class="button is-is-success is-outlined" v-if="recarga.last_status_value === 2">
