<template>
  <div>
    <div class="modal is-large" :class="showModalRechazar ? 'is-active' : '' ">
...
export default {
  ...
  computed:{
    ...mapGetters({
      errors:'recarga/ajustes/errorsRechazo',
      loadingRechazarReajuste:'recarga/ajustes/loadingRechazarReajuste',
    }),
    showModalRechazar:{
      get() {
    ...
    observacion:{
      get() {
        return this.$store.state.recarga.ajustes.observacion_rechazo;
      },
      set(newValue) {
        this.$store.commit('recarga/ajustes/SET_OBSERVACION_RECHAZAR', newValue);
      }
    },
    currentRouteName() {
