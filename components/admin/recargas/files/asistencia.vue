<template>
  <div>
    <div class="modal is-large" :class="openModalAsistencia ? 'is-active' : ''">
      <div class="modal-background" @click.prevent="hideAsistenciaModal"></div>
      <div class="modal-card" style="width: 80%;">
            ...
            <table class="table is-fullwidth">
              ...
              <tbody>
                <tr v-for="(columna, index) in columnas_asistencia" :key="index">
                  <td><input :disabled="columna.disabled" type="text" class="input is-rounded"
                      v-model="columna.nombre_columna" v-lowercase></td>
            ...
            <div class="column">
              <div class="field">
                <el-upload class="avatar-uploader" :action="`#`" :show-file-list="false"
                  ...
                  <div v-if="!file_asistencia" class="el-upload__text">Click para cargar archivo excel (Formato .XLSX)
                  </div>
                ...
                <template v-if="archivoAnalizadoCorrectamente">
                  <el-result icon="success" title="Archivo analizado correctamente"
                    :subTitle="`${asistencias.length} ${asistencias.length > 1 ? `registros analizados` : `registros analizado`}`"></el-result>
                </template>
                ...
                <template v-if="hasFileErrors">
                  <el-result icon="error" title="Error al analizar archivo"
                    :subTitle="`${totalFileErrors} ${totalFileErrors > 1 ? `errores` : `error`}`">
                    <template slot="extra">
          ...
          <div class="columns">
            <div class="column">
              <template v-if="asistencias.length">
                ...
                <span class="tag is-primary is-light">{{ `${asistencias.length} ${asistencias.length > 1 ? `registros` :
                  `registro` }`}}</span>
                <table class="table is-bordered is-striped is-narrow is-hoverable is-fullwidth">
                  ...
                  <tbody>
                    <tr v-for="(asistencia, index) in asistencias" :key="index">
                      <td v-for="(a, index) in asistencia" :key="index"><span
                          :class="(a === 'X' ? 'has-text-danger has-text-weight-bold' : (a === 'L' ? 'has-text-link' : ''))">{{ a }}</span>
                      </td>
                    </tr>
                  </tbody>
                </table>
        ...
        <footer class="modal-card-foot buttons is-right">
          <button v-if="paso === 3 && successImport" @click.prevent="hideAsistenciaModal"
          ...
          <button :disabled="!successImport" v-if="(paso === 2)" class="button is-primary is-rounded"
            @click.prevent="storeAsistencia" v-loading.fullscreen.lock="loadingSpinner">Cargar datos</button>
        </footer>
      </div>
    </div>
  </div>
...
export default {
  ...
  computed: {
    ...
    hasFileErrors() {
      return Array.isArray(this.errors_file) && this.errors_file.length > 0;
    },
    totalFileErrors() {
      return this.hasFileErrors ? this.errors_file.length : 0;
    },
    hasColumnError() {
      return Boolean(
        this.errorColumn &&
    ...
    archivoAnalizadoCorrectamente() {
      return (
        Array.isArray(this.asistencias) &&
    ...
    disabledButton() {
      ...
      return (
        ...
        !this.archivoAnalizadoCorrectamente
      );
    }
  },
  methods: {
    ...
    removeFile: function () {
      ...
      this.$store.commit('recargas/asistencia/SET_FILAS', {});
      this.$store.commit('recargas/asistencia/SET_ERROR_COLUMN', {});
      this.$store.commit('recargas/asistencia/SET_SUCCESS_IMPORT', false);
    },
    volver: function () {
